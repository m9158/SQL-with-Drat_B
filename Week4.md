...
<img width="1904" height="887" alt="image" src="https://github.com/user-attachments/assets/f7b42d38-81d9-46d8-b0ae-17362f42105c" />
...
~~~
<1번 문제>
오류 메시지 해석: 집계 함수 COUNT의 인자 수가 일치하지 않습니다.
코드 오류 발생 이유:
COUNT() 괄호 안에는 한개의 컬럼만 들어갈 수 있다. 현재 두개의 컬럼이 들어가 있어 오류가 발생할 수 밖에 없다.

=> *로 채워주거나, 한개만 설정해야 한다.

<2번 문제>
오류 메시지 해석:  SELECT 목록 식은 다음에서 그룹화하거나 집계되지 않은 열을 참조합니다.
코드 오류 발생 이유:
SELECT 컬럼 과정에서 AGE라는 컬럼을 선택했는데, GROUP BY를 하여 그룹화해주지 않아서 오류가 발생하였다. 해당 문제에서는 AGE를 그룹화하여 계산을하는 문제가 아니라,
사람이 단순히 몇명 존재하는지 물은 문제이다. 따라서 굳이 AGE 컬럼을 선택하는 코드를 사용할 필요가 없다. 따라서 SELECT 와 FROM 사이에 있는 AGE만 삭제해줘도 문제 해결이 될것이다. (GROUP BY AGE를 추가한다 하더라도, 문제 의도와 어긋남)
~~~
